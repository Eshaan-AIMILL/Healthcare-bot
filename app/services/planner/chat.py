...
async def chat_completions(request: Request, chat_request: ChatRequest) -> ChatResponse:
    logger.info(f"Incoming headers at Gateway: {request.headers}")
    ...
    if len(conversation_history) > 6:
        logger.info(f"Trimming conversation history to last 3 messages.")
    ...
    logger.info(f"Gateway Query: {latest_query!r} by user {security_context.user_id} ({security_context.enterprise_role})")
    ...
    try:
        ...
        logger.error(f"HTTP Graph execution failed: {exc}")
        response_text = (
            "The healthcare bot gateway encountered an internal error. "
